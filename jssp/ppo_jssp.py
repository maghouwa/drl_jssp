...
import numpy as np
from jssp.validation import validate
from jssp.jssp_env import SJSSP
from jssp.agent_utils import g_pool_cal, aggr_obs, eval_actions, select_action
from jssp.utils import uni_instance_gen
...
def main(n_j, n_m):
    ...
    dataLoaded = np.load('./DataGen/generatedData' + str(configs["n_j"]) + '_' + str(configs["n_m"]) + '_Seed' + str(
        configs["seed"]) + '.npy')
    vali_data = []
    ...
    for i_update in range(configs["max_updates"]):
        ...
        if i_update % 99 == 0:
            ...
            valid_dumps.append(vali_result)
